...
/**
 * 聊天窗口（editor） editor的生命周期：
 * 1.workbench初始化editor（使用默认的构造器），创建editor的地址（site），然后调用init() （editor
 * site允许editor访问workbench service)
 * 2.editor设为可见后，调用createControl方法创建editor的widget 3.editor创建后，会调用setFocus()
 * 4.editor关闭后，如果需要保存，doSave()会被调用 5.最后dispose()
 */
